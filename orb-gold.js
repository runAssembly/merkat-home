...
function _(e, n, s, r = 0.3) {
  ...
  for (const t of n) {
    const o = t.a ?? 1;
    if (o < 0.02) continue;
    const c = Math.min(1, Math.max(0, t.white)); const L = s ? 1 - c : c; const R = Math.round(169 + 77 * L), G = Math.round(127 + 71 * L), B = Math.round(54 + 49 * L); e.fillStyle = `rgba(${R},${G},${B},${o})`, e.beginPath(), e.arc(t.x, t.y, Math.max(r, t.r), 0, Math.PI * 2), e.fill();
  }
}
function $(e, n) {
  return (e / 300) ** n;
